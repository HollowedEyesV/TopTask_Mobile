...
function init() {
  ...
  function actualizarBotonSidebar() {
    ...
    if (esVistaMovil()) {
      ...
      sidebarToggle.title = sidebar?.classList.contains('collapsed') ? 'Mostrar menú' : 'Ocultar menú';
  ...
  mobileMenuBtn?.addEventListener('click', () => {
    if (sidebar?.classList.contains('mobile-open')) cerrarMenuMovil();
    else abrirMenuMovil();
  });

  mobileSidebarOverlay?.addEventListener('click', cerrarMenuMovil);

  document.querySelectorAll('.nav-btn').forEach(btn => {
    btn.addEventListener('click', () => {
      if (esVistaMovil()) cerrarMenuMovil();
    });
  });
